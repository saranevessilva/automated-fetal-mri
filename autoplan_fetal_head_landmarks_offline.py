...
def process_image(images, connection, config, metadata, im, state):
    ...
    for iImg in range(data.shape[-1]):
        ...
        if state["first_slice"] == 1:
            ...
            current_pos = position[1] + pixdim_z * slice
            state["slice_pos"] += current_pos
        ...
        if contrast == 1:
            # im[:, :, :] = np.squeeze(data[:, :, :])  # changing this for real-time application
            im[:, :, :] = np.squeeze(data[:, :])
        ...
        im_ = rotate(im_, angle=180, axes=(0, 1), reshape=False, order=1, mode='nearest')

        # Assuming 'im_' and the other variables are defined earlier in your code
        if slice == nslices - 1 and contrast == 1:  # Ensuring the first two echoes have been completely acquired
