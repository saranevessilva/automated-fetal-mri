...
def plot_centiles4(id, scan_date, ga, fetus, placenta, amniotic, efw, volumetry_path, scan_id):
    ...
    fig.write_image(volumetry_path + "/" + scan_id + "-graphs.png")
...
def process_image(images, connection, config, metadata):
    ...
    excel = "/home/sn21/volumetry-mibirth/mibirth-volumetry.csv"  # Replace with the path to your Excel file
    ...
    for index, row in excel.iterrows():
        ...
        scan_id = row['ID']
        ga = row['GA']
        print("Gestational Age:", ga)

        # Define the path you want to create
        new_directory_seg = volumetry_path + "/" + scan_id + "-nnUNet_seg-volumetry/"
        new_directory_pred = volumetry_path + "/" + scan_id + "-nnUNet_pred-volumetry/"
        ...
        # Construct the NIFTI file paths
        suffix = "_bssfp_cor_wholeuterus.nii.gz"
        im = f"/home/sn21/volumetry-mibirth/{scan_id}{suffix}"  # Replace with your image directory
        # fetal_im_sitk = im
        im = sitk.ReadImage(im)

        # Define the voxel sizes (spacing)
        voxel_sizes = (0.7, 0.7, 4.0)  # Voxel sizes in mm

        # Create the direction cosines matrix from srow_x, srow_y, srow_z
        srows = (srow_x[0], srow_x[1], srow_x[2], srow_y[0], srow_y[1], srow_y[2], srow_z[0], srow_z[1], srow_z[2])

        print("VOXEL SIZE", voxel_sizes)

        # Set spacing and direction for the image
        im.SetSpacing(voxel_sizes)
        im.SetDirection(srows)
        print("New spacing and direction have been set!")

        # Convert the image to a numpy array for processing (if needed)
        fetal_im_sitk = sitk.GetArrayFromImage(im)

        # If you need to permute axes, do so on the SimpleITK image (not the numpy array)
        im = sitk.PermuteAxes(im, [1, 2, 0])  # Switch axes if needed
        print("Size after transposition:", im.GetSize())

        # Convert the numpy array back to a SimpleITK image (after processing the spacing)
        fetal_im_sitk = sitk.GetImageFromArray(fetal_im_sitk)
        fetal_im_sitk.SetSpacing(voxel_sizes)  # Set the spacing again for the new SimpleITK image
        fetal_im_sitk.SetDirection(srows)  # Set the direction again for the new SimpleITK image

        sitk.WriteImage(fetal_im_sitk,
                        volumetry_path + "/" + scan_id + "-output.nii.gz")

        sitk.WriteImage(fetal_im_sitk,
                        volumetry_path + "/" + scan_id + "-nnUNet_seg-volumetry/Volumetry_001_0000.nii"
                                                           ".gz")
        ...
        terminal_command = (("export nnUNet_raw='/home/sn21/landmark-data/Volumetry/nnUNet_raw'; export "
                            ...
                            + scan_id + "-nnUNet_seg-volumetry/ -o " + volumetry_path + "/" + scan_id
        ...
        segmentation_filename = os.path.join(volumetry_path, scan_id + "-nnUNet_pred-volumetry",
        ...
        # img_name = volumetry_path + "/" + timestamp + "-nnUNet_seg-volumetry/Volumetry_001_0000.nii.gz"  # RT
        img_name = volumetry_path + "/" + scan_id + "-nnUNet_seg-volumetry/Volumetry_001_0000.nii.gz"
        ...
        plt.savefig(volumetry_path + "/" + scan_id + '-out-rad-grey.png')
        ...
        plt.savefig(volumetry_path + "/" + scan_id + '-out-rad-with-lab.png')
        ...
        plt.savefig(volumetry_path + "/" + scan_id + '-out-rad-with-lab-coronal.png')
        ...
        res_lab_raw = resample_to_isotropic(lab_name, volumetry_path + "/" + scan_id + "-res_lab.nii.gz")
        ...
        pio.write_image(fig, volumetry_path + "/" + scan_id + '-fetus_3D.png')

        efw = fetus * 1.031 + 0.12

        plot_centiles4(id, scan_date, ga, fetus, placenta, amniotic, efw, volumetry_path, scan_id)
        ...
        img = io.imread(volumetry_path + "/" + scan_id + '-out-rad-grey.png')
        # Check and convert boolean array if necessary
        if img.dtype == bool:
            img = img.astype(np.uint8)  # Convert boolean to uint8 if needed
        figm1 = px.imshow(img)
        # figm1 = io.imread(volumetry_path + "/" + scan_id + '-out-rad-grey.png')

        img = io.imread(volumetry_path + "/" + scan_id + '-out-rad-with-lab.png')
        # Check and convert boolean array if necessary
        if img.dtype == bool:
            img = img.astype(np.uint8)  # Convert boolean to uint8 if needed
        figm2 = px.imshow(img)
        # figm2 = io.imread(volumetry_path + "/" + scan_id + '-out-rad-with-lab.png')

        img = io.imread(volumetry_path + "/" + scan_id + '-fetus_3D.png')
        # Check and convert boolean array if necessary
        if img.dtype == bool:
            img = img.astype(np.uint8)  # Convert boolean to uint8 if needed
        figm3 = px.imshow(img)
        # figm3 = io.imread(volumetry_path + "/" + scan_id + '-fetus_3D.png')
        ...
        fig.write_image(volumetry_path + "/" + scan_id + "-report1.png")

        img = io.imread(volumetry_path + "/" + scan_id + '-graphs.png')
        # Check and convert boolean array if necessary
        if img.dtype == bool:
            img = img.astype(np.uint8)  # Convert boolean to uint8 if needed
        figm5 = px.imshow(img)
        # figm5 = io.imread(volumetry_path + "/" + scan_id + '-graphs.png')
        ...
        fig2.write_image(volumetry_path + "/" + scan_id + "-report2.png")
        ...
        f_name_summary_intro = volumetry_path + "/" + scan_id + "-report1.png"
        f_name_summary_graphs = volumetry_path + "/" + scan_id + "-report2.png"

        # f_name_summary_intro = proc_dir + '/out-summary-intro.png'

        f_name_summary_intro_pdf = volumetry_path + "/" + scan_id + '-test-summary-intro.pdf'
        f_name_summary_graphs_pdf = volumetry_path + "/" + scan_id + '-test-summary-graphs.pdf'
        ...
        output_report_name_pdf = volumetry_path + "/" + scan_id + '-out-report-combined.pdf'
