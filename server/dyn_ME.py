        ...
        for item in connection:
            ...
            # ----------------------------------------------------------
                ...
                if item.image_series_index != currentSeries:
                    ...
                    image = process_image(imgGroup, connection, config, metadata, data_echoes)
                    connection.send_image(image)
                    imgGroup = []
...
def process_raw(group, connection, config, metadata):
    ...
    imagesOut = process_image(imagesOut, connection, config, metadata,data_echoes)

    return imagesOut


def process_image(images, connection, config, metadata,data_echoes):
    if len(images) == 0:
        return []
    ...
    for iImg in range(data.shape[-1]):
        ...
        print("Maximum repetitions are", nrepetitions)
        contrast = oldHeader.contrast + 1
        print("contrasts is", contrast-1)
        ...
        #if slice == 0:
        #    data_echoes[:, :, :, :, contrast -1] = data[...,iImg]
        #else:
        #    data_echoes[:, :, :, :, ncontrasts * slice + contrast -1] = data[...,iImg]

        #if(slice == nslices - 1 and contrast == ncontrasts -1):
            #sitk.WriteImage(sitk.GetImageFromArray(np.transpose(np.squeeze(data_echoes),[2,1,0])), "/home/sn21/prova_image.nii.gz")
        #    T2s_mapping(data_echoes, images, connection, config, metadata, oldHeader)
        #    continue
...
def T2s_mapping(data_echoes, images, connection, config, metadata, oldHeader):
    ...
    sitk.WriteImage(im_echo1, case_path + "/NIFTI/Freemax_001_0000.nii.gz")
    ...
    # Construct command
    commands_to_execute = (
        f"{mrconvert_path} {case_path}/NIFTI/Freemax_001_0000.nii.gz "
        f"{case_path}/nnUNet_image/Freemax_001_0000.nii.gz "
        "-strides -1,3,2 -force"
    )
    ...
    create_dir(case_path + "/nnUNet_placenta_pred")
    ...
    commands_to_execute = ("export nnUNet_raw='/home/jv21/Desktop/nnUNet_folder/nnUNet_raw';"
                           ...
                           "nnUNetv2_predict -i " + case_path + "/nnUNet_image/ -o " + case_path + "/nnUNet_placenta_pred/ -d 004 -c 3d_fullres -f 1")
    full_command = f"{terminal_command} -- bash -c '{commands_to_execute}'"
    os.system(full_command)

    # Fetal Brain
    create_dir(case_path + "/nnUNet_brain_pred")
    create_dir(case_path + "/nnUNet_brain_PP")
    terminal_command = "gnome-terminal"
    commands_to_execute = ("export nnUNet_raw='/home/jv21/Desktop/nnUNet_folder/nnUNet_raw';"
                           "export nnUNet_preprocessed='/home/jv21/Desktop/nnUNet_folder/nnUNet_preprocessed' ; "
                           "export nnUNet_results='/home/jv21/Desktop/nnUNet_folder/nnUNet_results'; "
                           "conda activate gadgetron ; "
                           "nnUNetv2_predict -i " + case_path + "/nnUNet_image/ -o " + case_path + "/nnUNet_brain_pred/ -d Dataset602_brainsv2 -c 3d_fullres -f 0 1 2 3 4 --disable_tta;"
                                                                                                   "nnUNetv2_apply_postprocessing -i" + case_path + "/nnUNet_brain_pred/ -o" + case_path + "/nnUNet_brain_PP/" + " -pp_pkl_file /home/jv21/Desktop/nnUNet_folder/nnUNet_results/Dataset602_brainsv2/nnUNetTrainer__nnUNetPlans__3d_fullres/crossval_results_folds_0_1_2_3_4/postprocessing.pkl -np 8 -plans_json /home/jv21/Desktop/nnUNet_folder/nnUNet_results/Dataset602_brainsv2/nnUNetTrainer__nnUNetPlans__3d_fullres/crossval_results_folds_0_1_2_3_4/plans.json")
    full_command = f"{terminal_command} -- bash -c '{commands_to_execute}'"
    os.system(full_command)
    ...
    commands_to_execute1 = (
        f"{mrconvert_path} {case_path}/nnUNet_brain_PP/Freemax_001.nii.gz "
        f"{case_path}/nnUNet_brain_PP/Freemax_001.nii.gz "
        "-strides -2,-1,-3 -force"
    )

    commands_to_execute2 = (
        f"{mrconvert_path} {case_path}/nnUNet_placenta_pred/Freemax_001.nii.gz "
        f"{case_path}/nnUNet_placenta_pred/Freemax_001.nii.gz "
        "-strides -2,-1,-3 -force"
    )
    ...
    segmentation_brain = sitk.GetArrayFromImage(sitk.ReadImage(case_path + "/nnUNet_brain_PP/Freemax_001.nii.gz"))
    seg = sitk.GetImageFromArray(segmentation_brain)
    sitk.WriteImage(seg, case_path + "/NIFTI/image_seg.nii.gz")
    ...
    for i in range(ncontrasts):
        ...
        seg = sitk.GetImageFromArray(segmentation_brain)
        sitk.WriteImage(not_cropped, case_path + "/NIFTI/image_echo_nc_brain_" + str(i) + ".nii.gz")
        sitk.WriteImage(crop_image, case_path + "/NIFTI/image_echo_brain_" + str(i) + ".nii.gz")
        flat_array = np.ndarray.flatten(image_echoes_b[:, :, :, i])
        cropped_brain[i, :] = flat_array

    # Crop images (all echoes) to placenta
    segmentation_placenta = sitk.GetArrayFromImage(sitk.ReadImage(case_path + "/nnUNet_placenta_pred/Freemax_001.nii.gz"))
    ...
    for i in range(ncontrasts):
        ...
        sitk.WriteImage(not_cropped, case_path + "/NIFTI/image_echo_nc_placenta_" + str(i) + ".nii.gz")
        sitk.WriteImage(crop_image, case_path + "/NIFTI/image_echo_placenta_" + str(i) + ".nii.gz")
    ...
    sitk.WriteImage(image_t2s_brain, case_path + "/fbrain_t2smap.nii.gz")
    ...
    sitk.WriteImage(image_t2s_placenta, case_path + "/placenta_t2smap.nii.gz")
