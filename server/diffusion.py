...
def process(connection, config, metadata):
    ...
    data_diff = np.zeros((dim_x, dim_y, 1, 1, nslices*ncontrasts*nrepetitions))
        ...
        for item in connection:
            ...
            # ----------------------------------------------------------
                ...
                if item.image_series_index != currentSeries:
                    ...
                    image = process_image(imgGroup, connection, config, metadata, data_diff)
                    connection.send_image(image)
                    imgGroup = []
...
def process_raw(group, connection, config, metadata):
    ...
    imagesOut = process_image(imagesOut, connection, config, metadata,data_diff)

    return imagesOut


def process_image(images, connection, config, metadata,data_diff):
    if len(images) == 0:
        return []
    ...
    for iImg in range(data.shape[-1]):
        ...
        if (slice == nslices - 1 and contrast == ncontrasts - 1 and repetition == ncontrasts-1):
            data_diff = data
            print(data_diff.shape)
            Diff_analysis(data_diff, images, connection, config, metadata, oldHeader)
            continue
...
def Diff_analysis(data_diff, images, connection, config, metadata, oldHeader):

    ncontrasts = metadata.encoding[0].encodingLimits.contrast.maximum + 1
    nslices = metadata.encoding[0].encodingLimits.slice.maximum + 1
    print("repetitions are", oldHeader)

    # Create array with dimensions (x,y,nslices,ncontrasts)
    image_diff = np.zeros((data_diff.shape[0], data_diff.shape[1], nslices, ncontrasts*ncontrasts))

    for i in range(9):
        image_diff[:,:,:,i] = np.squeeze(data_diff[:,:,:,:,nslices*i:nslices*(i+1)])

    affine = np.eye(4)
    im_e1dir1= nib.Nifti1Image(image_diff,affine)
    nib.save(im_e1dir1,'/home/sn21/data/t2-stacks/diffusion_try.nii.gz')
