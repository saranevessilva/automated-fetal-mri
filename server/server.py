...
import analyzeflow
import diffusion
import diffusion_base
import diffusion_base_reacq
import dynamicME
...
import fire_svrtk_integration
import automated_svrtk_launch
import automated_pull_svr_brain
import automated_pull_svr_body
import mrd2dicom
import automated_t2s_mapping
...
class Server:
    ...
    def handle(self, sock):
        ...
        try:
            ...
            elif config == "fire_svrtk_integration":
                logging.info("Starting fire_svrtk_integration processing based on config")
                fire_svrtk_integration.process(connection, config, metadata)
            ...
            elif config == "diffusion_base":
                logging.info("Starting diffusion_base processing based on config")
                diffusion_base.process(connection, config, metadata)
            elif config == "diffusion_base_reacq":
                logging.info("Starting diffusion_base_reacq processing based on config")
                diffusion_base_reacq.process(connection, config, metadata)
            elif config == "dynamicME":
                logging.info("Starting dynamicME processing based on config")
                dynamicME.process(connection, config, metadata)
            elif config == "automated_svrtk_launch":
                logging.info("Starting automated_svrtk_launch processing based on config")
                automated_svrtk_launch.process(connection, config, metadata)
            elif config == "automated_t2s_mapping":
                logging.info("Starting automated_t2s_mapping processing based on config")
                automated_t2s_mapping.process(connection, config, metadata)
