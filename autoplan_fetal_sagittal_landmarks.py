...
def process_image(images, connection, config, metadata):
    ...
    fetalbody_path = ("/home/sn21/miniconda3/envs/gadgetron/share/gadgetron/python/results/fetal-sagittal/"
                      + date_path)
    ...
    landmarks_path = ("/home/sn21/miniconda3/envs/gadgetron/share/gadgetron/python/results/fetal-sagittal/"
                      + date_path)
    os.mkdir(fetalbody_path + "/" + timestamp + "-nnUNet_seg-fetal-sagittal/")
    os.mkdir(fetalbody_path + "/" + timestamp + "-nnUNet_pred-fetal-sagittal/")

    fetal_im_sitk.SetSpacing(voxel_sizes)
    sitk.WriteImage(cropped, fetalbody_path + "/" + timestamp
                    + "-nnUNet_seg-fetal-sagittal/FetalLungApices_001_0000.nii.gz")  # Sara!
    ...
    start_time = time.time()
    # new network
    terminal_command = (("export nnUNet_raw='/home/sn21/landmark-data/FetalLungApices/nnUNet_raw'; "
                         "export nnUNet_preprocessed='/home/sn21/landmark-data/FetalLungApices"
                         "/nnUNet_preprocessed' ; export "
                         "nnUNet_results='/home/sn21/landmark-data/FetalLungApices/nnUNet_results' ; "
                         "conda activate gadgetron ; nnUNetv2_predict -i ") + landmarks_path + "/" +
                        timestamp + "-nnUNet_seg-fetal-sagittal/ -o " + landmarks_path + "/" + timestamp +
                        "-nnUNet_pred-fetal-sagittal/ -d 086 -c 3d_fullres -f 1")
    ...
    landmarks_path = os.path.join(fetalbody_path, timestamp + "-nnUNet_pred-fetal-sagittal/FetalLungApices_001.nii.gz")
    ...
    # new network
    apices = (landmark == 1.0).astype(int)

    nonzero_indices = np.nonzero(apices)
    ...
    closed_segmentation = ndimage.binary_closing(apices,
    ...
    apex_1 = (labelled_segmentation == 1.0).astype(int)
    apex_2 = (labelled_segmentation == 2.0).astype(int)

    print("Unique labels in labelled_segmentation:", np.unique(labelled_segmentation))

    apex_1 = np.array(ndimage.center_of_mass(apex_1))
    apex_2 = np.array(ndimage.center_of_mass(apex_2))
    mid = ((apex_1[0] + apex_2[0]) / 2), ((apex_1[1] + apex_2[1]) / 2), ((apex_1[2] + apex_2[2]) / 2)

    print("LANDMARKS", apex_1, apex_2, mid)

    apex_1 = (apex_1[0] + lower_left_corner[0], apex_1[1] + lower_left_corner[1],
              apex_1[2] + lower_left_corner[2])

    apex_2 = (apex_2[0] + lower_left_corner[0], apex_2[1] + lower_left_corner[1],
              apex_2[2] + lower_left_corner[2])

    mid = (mid[0] + lower_left_corner[0], mid[1] + lower_left_corner[1],
           mid[2] + lower_left_corner[2])

    # Print the center y value and its corresponding coordinates
    # print("Coordinates of centre y value (x, y, z):", mid)
    ...
    text_file_1 = ("/home/sn21/miniconda3/envs/gadgetron/share/gadgetron/python/results/fetal-sagittal/" +
                   date_path + "/" + timestamp + "-com_fetal-sagittal.txt")
    text_file = "/home/sn21/freemax-transfer/Sara/landmarks-interface-autoplan/sara_fetalbody.dvs"
    ...
    apex_1 = pixdim_x * apex_1[0], pixdim_y * apex_1[1], pixdim_z * apex_1[2]
    apex_2 = pixdim_x * apex_2[0], pixdim_y * apex_2[1], pixdim_z * apex_2[2]
    mid = pixdim_x * mid[0], pixdim_y * mid[1], pixdim_z * mid[2]
    ...
    apex_1 = np.round(apex_1, 3)
    apex_2 = np.round(apex_2, 3)
    mid = np.round(mid, 3)

    print("POSITION MM", position)
    print("APEX 1 MM", apex_1)
    print("APEX 2 MM", apex_2)
    print("MID MM", mid)

    apex_1 = (apex_1[0] - centreofimageposition[0],
              apex_1[1] - centreofimageposition[1],
              apex_1[2] - centreofimageposition[2])  # JAV

    apex_2 = (apex_2[0] - centreofimageposition[0],
              apex_2[1] - centreofimageposition[1],
              apex_2[2] - centreofimageposition[2])

    mid = ((mid[0]) - centreofimageposition[0],
           (mid[1]) - centreofimageposition[1],
           mid[2] - centreofimageposition[2])

    print("centreofimageposition", centreofimageposition)
    print("APEX 1 OFFSET", apex_1)
    print("APEX 2 OFFSET", apex_2)
    ...
    apex_1 = (apex_1[0], apex_1[2], apex_1[1])
    apex_2 = (apex_2[0], apex_2[2], apex_2[1])
    mid = (mid[0], mid[2], mid[1])

    apex_1 = (apex_1[0] + position[0],
              apex_1[1] + position[1],
              apex_1[2] + position[2])

    apex_2 = (apex_2[0] + position[0],
              apex_2[1] + position[1],
              apex_2[2] + position[2])

    mid = ((mid[0]) + position[0],
           (mid[1]) + position[1],
           mid[2] + position[2])

    # WRITE AN IF STATEMENT HERE. Sara!
    # Find the indices where cm_cereb is NaN
    idx_apex_1 = np.isnan(apex_1)
    # Use numpy.where to replace NaN values with corresponding values from cm_brain
    apex_1 = np.where(idx_apex_1, (centreofimageposition[0], centreofimageposition[1],
                                   centreofimageposition[2]), apex_1)

    # Find the indices where cm_cereb is NaN
    idx_apex_2 = np.isnan(apex_2)
    # Use numpy.where to replace NaN values with corresponding values from cm_brain
    apex_2 = np.where(idx_apex_2, (centreofimageposition[0], centreofimageposition[1],
                                   centreofimageposition[2]), apex_2)

    # Find the indices where cm_cereb is NaN
    idx_mid = np.isnan(mid)
    # Use numpy.where to replace NaN values with corresponding values from cm_brain
    mid = np.where(idx_mid, (centreofimageposition[0], centreofimageposition[1],
                             centreofimageposition[2]), mid)

    apex_1 = (apex_1[0], apex_1[1], -apex_1[2])
    apex_2 = (apex_2[0], apex_2[1], -apex_2[2])
    mid = (mid[0], mid[1], -mid[2])
    position = (position[0], position[1], position[2])

    print("APEX 1 ROT", apex_1)
    print("APEX 2 ROT", apex_2)
    print("MID ROT", mid)
    ...
    output_image_path = landmarks_path.replace(".nii.gz", "_2-labels.nii.gz")
