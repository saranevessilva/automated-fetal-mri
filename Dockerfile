...
RUN mkdir -p /opt/code && \
    cd /opt/code && \
    git clone --single-branch --branch bubu https://github.com/kspacekelvin/python-ismrmrd-server.git && \
    git clone --single-branch --branch bubu https://github.com/saranevessilva/automated-fetal-mri.git && \
    git clone --single-branch --branch bubu https://github.com/ismrmrd/ismrmrd-python-tools.git && \
    cd /opt/code/ismrmrd-python-tools && \
    pip3 install --no-cache-dir . && \
    pip freeze
